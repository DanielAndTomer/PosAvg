        ...
        def onClick_start ():
            ...
            if globalFlag==True:
                if value>0:
                    ...
                    else:   controller.show_frame(Stopped)
...
class Done(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text="All Done!!!\n"
                                    "Take a look on the log file to see the location\n")
        label.config(bg="white", fg="orange", font="Times 14 bold")
        label.place(x=120, y=95)

        load = Image.open("Images/vpic.png")
        load=load.resize((110,110))
        render = ImageTk.PhotoImage(load)
        # labels can be text or images
        img = tk.Label(self, image=render)
        img.image = render
        img.place(x=250, y=160)
